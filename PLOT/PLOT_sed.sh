...
CPT=cptfile.cpt
CPT2=cptfile2.cpt
gmt makecpt -Cjet -T0/1.5/0.3 -D -Ic -Z > cptfile.cpt
...
Plot_dot()
{
    ...
    for sta in `awk '$3=="phzh" || $3=="phzhwf" {print $4}' /mnt/home/jieyaqi/Documents/invinfo.txt`
    do
    ...
    gmt psscale -C$CPT -D$DSCALE -K -O -B+l'Sedimentary thickness (km)' >> $PS
}
...
Plot_map()
{
    ...
    awk '$3=="phzh" || $3=="phzhwf" {print $1, $2}' /mnt/home/jieyaqi/Documents/invinfo.txt | gmt psxy -R$R -J$J -Sc8p -W0.5p,black -K -O >> $PS
...
{   
    ...
    gmt surface $INPUT_FILE -R$R -I0.1 -Ginput.grd -T0.5
    gmt grdimage input.grd -R -J$J -C$CPT -O -K -t80 -Icut.grd.gradient2 -X$3 -Y$4 >> $PS
    gmt psclip -R$R -J$J ~/Documents/mask.xy -S10 -K -O -V >> $PS
    gmt grdimage input.grd -R -J$J  -C$CPT -Icut.grd.gradient2 -K -O >> $PS
    gmt psclip -C -O -K >> $PS
    gmt pscoast -R$R -J$J -W0.5p,darkgrey -A1000 -K -O >> $PS
...
Plot_dot
Plot_map
awk '{print $1, $2, $6}' $INPUT_FILE2 > temp
Plot_uncert temp 0/1.5/0.3 3.5i 0i "@~s@~ (km)"
awk '{print $1, $2, $10}' $INPUT_FILE2 > temp
Plot_uncert temp 0/5/1 3.5i 0i "@~s@~ / thickness (km)"
awk '{print $1, $2, $11}' $INPUT_FILE2 > temp
Plot_uncert temp 0/1.5/0.3 3.5i 0i "thickness - @~s@~ (km)"
awk '{print $1, $2, $12}' $INPUT_FILE2 > temp
Plot_uncert temp 0/1.5/0.3 3.5i 0i "thickness + @~s@~ (km)"
