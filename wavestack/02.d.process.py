...
path = "/mnt/ufs18/nodr/home/jieyaqi/waveformstack/SAC"
...
def downsample(trace):
    freq = int(trace.stats.sampling_rate/10)
...
def cut_trace(trace):
    arrival = model.get_travel_times(source_depth_in_km=trace.stats.sac.evdp,distance_in_degree=trace.stats.sac.gcarc,phase_list=["P"])
    try:
        b = np.floor(arrival[0].time-300)
    except IndexError:
        return None
    e = np.ceil(arrival[0].time+500)
    arrivaltime = trace.stats.starttime + 300 + arrival[0].time
    trace.stats.sac['nzyear'] = arrivaltime.year
    trace.stats.sac['nzjday'] = arrivaltime.julday
    trace.stats.sac['hour'] = arrivaltime.hour
    trace.stats.sac['nzmin'] = arrivaltime.minute
    trace.stats.sac['nzsec'] = arrivaltime.second
    trace.stats.sac['nzmsec'] = arrivaltime.microsecond/1000
        ...
        try:
            ...
            for trace in tr:
                ...
                if trace.stats.sac["kuser1"]=='False':
                    ...
                    # trace = remove_response(trace,sac)
                    # if trace == None:
                    ...
                    logger.debug(f'{sac} is processed')
    return
